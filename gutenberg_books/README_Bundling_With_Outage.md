...
## 13) Outage Impact Extension (Planned Outage Burden)

**Goal.** In addition to crew travel savings, quantify how bundling can **reduce planned outage burden**—i.e., fewer separate de-energizations / gas shutdowns in the same neighborhood. This adds **Outage Minutes Avoided** and **Customer Minutes Avoided (CMI)** metrics, optional $ monetization, and an optional score boost.

### 13.1 New Data Fields (add to `WorkOrders` or join from OMS/GIS)
- `OutageRequired` (Y/N)  
- `OutageType` (Electric | Gas)  
- `OutageZoneID` (feeder/switch block, or gas isolation/CP zone)  
- `ExpectedOutageMinutes` (number)  
- `CustomersAffectedEst` (number)  
- `CanShareWindow` (Y/N/Unknown) — compatibility of switching/isolation plans

> If you lack OMS integration, start with conservative defaults per asset/zone (e.g., typical minutes and customers).

### 13.2 New Parameters (What-If / Manage Parameters)
Create these to keep the model tunable (defaults shown):

| Parameter (new) | Type | Range/Step | Default | Purpose |
|---|---|---:|---:|---|
| `OutageShareFactor` | **Power Query** Decimal | 0–1 (0.05) | **0.7** | Fraction of the *second* outage avoided when sharing a window |
| `CostPerOutageMinute` | **What-If** Decimal | 0–50 (0.5) | **0** | Internal $/min for planned outage ops (optional) |
| `CostPerCustomerMinute` | **What-If** Decimal | 0–2 (0.05) | **0** | Optional proxy $/customer-minute (set to 0 if not monetizing) |
| `TotalCustomersServed` | **What-If** Whole | 1–2,000,000 (1000) | **100,000** | Denominator for context metric (Planned SAIDI-equivalent) |
| `Lambda_OutageWeight` | **What-If** Decimal | 0–1 (0.05) | **0.30** | Weight to add outage minutes (→ hours) into score |
| `Lambda_CMIWeight` | **What-If** Decimal | 0–1 (0.05) | **0.10** | Weight to add CMI (→ hours) into score |
| `AvgCustomersPerCrew` | **What-If** Whole | 1–500 (1) | **50** | Converts CMI to “crew-hours” scale (optional in score) |

### 13.3 Power Query (M): Pair-level Outage Columns
In **`BundleSuggestions_Live`** (or your pair table), add these **Custom Columns**:

```m
// Same outage zone & need
SameOutageZone   = if [AnchorOutageZoneID] = [CandidateOutageZoneID] then 1 else 0,
BothNeedOutage   = if [AnchorOutageRequired] = "Y" and [CandidateOutageRequired] = "Y" then 1 else 0,
Shareable        = if [SameOutageZone] = 1 and [BothNeedOutage] = 1 and [CandidateCanShareWindow] <> "N" then 1 else 0,

// Outage minutes avoided (conservative)
OutageMinutesAvoided =
    if [Shareable] = 1 then
        Number.From([CandidateExpectedOutageMinutes]) * Number.From(OutageShareFactor)
    else 0,

// Customer-minutes avoided (CMI)
CustomerMinutesAvoided =
    if [Shareable] = 1 then
        Number.From([OutageMinutesAvoided]) * Number.From([CandidateCustomersAffectedEst])
    else 0
```

> Note: `OutageShareFactor` above is a **Power Query parameter** (Manage Parameters) so PQ can evaluate it at refresh time. Keep your existing distance/penalty/score logic as-is.

### 13.4 DAX Measures (add to **Measures – Bundling**)

```DAX
// Core outage impact
[Outage Minutes Avoided] :=
SUM ( BundleSuggestions_Live[OutageMinutesAvoided] )

[Customer Minutes Avoided] :=
SUM ( BundleSuggestions_Live[CustomerMinutesAvoided] )

// Optional $ monetization (leave the $ parameters at 0 if you prefer non-$ metrics)
[Outage Cost Saved ($)] :=
[Outage Minutes Avoided] * [CostPerOutageMinute Value]

[Customer Impact Saved ($)] :=
[Customer Minutes Avoided] * [CostPerCustomerMinute Value]

// Composite savings: crew travel + outage burden
[Total Savings ($)] :=
[CostSaved ($)] + [Outage Cost Saved ($)] + [Customer Impact Saved ($)]

// Context-only metric (not official SAIDI)
[Δ Planned SAIDI (mins/customer)] :=
DIVIDE ( [Customer Minutes Avoided], [TotalCustomersServed Value] )
```

**Optional: score with outage context**
```DAX
[BundleScore+] :=
VAR travel    = [TimeSaved (hrs)]
VAR penalty   = [Penalty (hrs)]
VAR outage_hr = DIVIDE( [Outage Minutes Avoided], 60.0 )
VAR cmi_hr    = DIVIDE( [Customer Minutes Avoided], 60.0 * [AvgCustomersPerCrew Value] )
RETURN
MAX ( 0, travel - penalty
          + [Lambda_OutageWeight Value] * outage_hr
          + [Lambda_CMIWeight Value]    * cmi_hr )
```

> Keep your original **[BundleScore]** as the default rank and expose **[BundleScore+]** as an alternate sort (toggle with a button/bookmark if you like).

### 13.5 Visuals to Add (quick win)
- **Cards:** *Outage Minutes Avoided*, *Customer Minutes Avoided*, *Δ Planned SAIDI (mins/customer)*, *Outage Cost Saved ($)*, *Total Savings ($)*  
- **Bar by OutageZoneID:** sum *Outage Minutes Avoided* and *CMI* to spotlight zones with the biggest double-scheduling wins  
- **Matrix (Anchor → Candidates):** include `Shareable`, `ExpectedOutageMinutes`, `CustomersAffectedEst`  
- **Tooltip note:** “Planned SAIDI-equivalent is a context metric; not an official reliability KPI.”

### 13.6 Approvals (optional text lines)
Append these lines into your Teams Approval body so approvers see the outage impact:
- **Outage shareable:** Yes/No  
- **Est. outage minutes avoided:** X  
- **Est. customer-minutes avoided:** Y

### 13.7 Governance (judge-friendly)
- Treat **Planned SAIDI-equivalent** as **context** (not official SAIDI/SAIFI).  
- Only mark `Shareable=1` when *Same zone* and *Both need outage* **and** ops marks `CanShareWindow ≠ "N"`.  
- Use slicers or a toggle to include/exclude outage monetization in *Total Savings ($)*.
